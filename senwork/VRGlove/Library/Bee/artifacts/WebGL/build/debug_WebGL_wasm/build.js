 ...
 navigator.bluetooth.requestDevice(options).then(function(device) {
  ...
  characteristic.addEventListener("characteristicvaluechanged", function(ev) {
   ...
   if (recvValue.charAt(0) == "p") {
    ...
    console.log("p=", p, " r=", r);
    SendMessage(target, "OnGloveAngleChanged", JSON.stringify({
     p: p,
     r: r
    }));
   }
   if (recvValue.charAt(0) == "f") {
    ...
    SendMessage(target, "OnGloveFingureChanged", JSON.stringify({
     f1: f1,
     f2: f2
    }));
   }
...
function _Disconnect(targetName) {
...
function _IsConnected(targetName) {
...
if (!Object.getOwnPropertyDescriptor(Module, "devices")) Module["devices"] = function() {
 abort("'devices' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");
};
